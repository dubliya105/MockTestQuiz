...
function ShowUser() {
  ...
  const hendleGetUsers=async()=>{
    try {
    if(auth){
      const result = await axios.get(`http://localhost:8080/api/user/userList`,{  
        headers:{
  ...
  const searchUser=async(e)=>{
    ...
    if(search!==''){
     const result = await axios.get(`http://localhost:8080/api/user/search/${search}`,{  
      headers:{
        Authorization: `Bearer ${auth}` 
      },
      params:{
   ...
   const hendleDeleteUser=async(id)=>{
    ...
    const decryptPass=async(item)=>{
      setView(item);
      try {
        const result = await axios.get(`http://localhost:8080/api/user/pass/${item._id}`) 
          ...
          <div className="row">
            ...
            <ul className="pagination justify-content-end">
              <li  className="page-item">
                <Link className={`page-link ${offset===1?'disabled':''}`} onClick={()=>setOffset(offset!==0?offset-1:1)}>&laquo;</Link>
              </li>
              {page.map((item,index)=>{
                return(
                        <li className="page-item" key={index} onClick={()=>setOffset(item)}>
              ...
              <li className={`page-item ${offset===totalPages?'disabled':''}`}  onClick={()=>setOffset(offset!==totalPages?offset+1:totalPages)}>
                <Link className="page-link" >
                  &raquo; 
                </Link>
              </li>
